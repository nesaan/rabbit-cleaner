    ...
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
		if (cleaned) {
			chrome.tabs.executeScript(
				tabs[0].id,
				{code: 'var vetoed = ["social", "toolbar", "barRight", "controls", "banner", "tray"]; for (let i = 0; i < vetoed.length ;i++) {document.getElementsByClassName(vetoed[i])[0].style.display = null;} document.getElementById("rabbitapp").className += " toolbarVisible";document.getElementById("rabbitapp").focus();'
			});
		}
		else {
			chrome.tabs.executeScript(
